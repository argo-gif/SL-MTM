...
class DashboardApp {
  ...
  private updateCrossFilterBadge(): void {
    ...
    if (activeFilterName) {
      ...
      badge.innerHTML = `🔍 Filter Aktif: <strong>${activeFilterName}</strong> <span style="margin-left: 4px; background: rgba(0,0,0,0.08); padding: 1px 6px; border-radius: 8px;">(Klik luar / di sini untuk melepas ✕)</span>`;
